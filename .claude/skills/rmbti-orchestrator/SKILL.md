...
**공개 사이트 반영(선택):** 웹을 재빌드해 `부동산투자성향테스트.html`이 바뀌었으면, 공개 사이트(GitHub Pages)를 갱신하려면 `publish-site` 스킬(`scripts/publish.sh`)로 index.html 동기화·푸시한다. 푸시는 외부 공개 행위이므로 **사용자가 배포를 원할 때만** 실행한다.
